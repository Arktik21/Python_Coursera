...
def get_car_from_csv_row(row):
    ...
    if (car_type not in CAR_TYPES.values()) and \
            (not (brand and photo_fn and carrying)) and\
            (os.path.splitext(photo_fn)[1] not in PHOTO_FILE_EXTENSIONS):
        return None
    ...
    elif car_type == CAR_TYPES['Truck']:
        return Truck(brand, photo_fn, carrying, body_whl)

    elif car_type == CAR_TYPES['SpecMachine']:
        if not extra:
            return None
